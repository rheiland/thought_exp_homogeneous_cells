...
figure(1)
bins = 0 : 0.01*birth_rate : 2*birth_rate; 

[N,temp] = hist( rates , bins ); 
plot( bins , N/sum(N) , 'b', 'linewidth',  3 ); 
...
[N,X] = hist( DT ); 
plot( X , N/sum(N), 'b' , 'linewidth', 2 ); 
N_theor = exp( -r*X ); 
hold on
plot( X , N_theor/sum(N_theor) ,'b:', 'linewidth', 3 ); 
hold off 
...
rates_hom = rates; 
division_times_hom = division_times; 
N_DT_hom = N; 
